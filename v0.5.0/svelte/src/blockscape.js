...
export function initBlockscape(featureOverrides = {}, { host = document } = {}) {
  ...
  const toolbarCopyButton = byId("toolbarCopy");
  const toolbarPasteButton = byId("toolbarPaste");
  ...
  function extractUrlCandidate(value) {
    ...
    // Recover common LLM mistakes such as "[label](logos/example.svg)".
    const markdownLinkMatch = trimmed.match(/^\[[^\]]*\]\(([^)\s]+)\)$/);
    if (markdownLinkMatch) return markdownLinkMatch[1];

    // Recover markdown-style autolinks such as "<logos/example.svg>".
  ...
  // Toolbar Copy – copies active map (or full series if active model is a series)
  if (toolbarCopyButton) {
    toolbarCopyButton.addEventListener("click", async () => {
      ...
      const active = models[activeIndex];
      const seriesJson = getActiveSeriesJson();
      const text = seriesJson || JSON.stringify(active.data, null, 2);
      const copied = await writeTextToClipboard(text);
      if (copied) {
        alert(seriesJson ? "Series copied to clipboard." : "Map copied to clipboard.");
      } else {
        window.prompt("Copy this JSON manually:", text);
      }
    });
  ...
  // Toolbar Paste – reads clipboard and appends any maps/series found
  if (!isReadOnly && toolbarPasteButton) {
    toolbarPasteButton.addEventListener("click", async () => {
      let text;
      try {
        text = await readTextFromClipboard();
      } catch (err) {
        console.warn("[Blockscape] clipboard read failed", err);
        alert("Unable to read from the clipboard. Use Cmd/Ctrl+V instead.");
        return;
      }
      if (!text || !text.trim()) {
        alert("Clipboard is empty.");
      ...
      try {
        ...
        console.warn("[Blockscape] toolbar paste: invalid JSON", err);
        alert("Clipboard does not contain valid Blockscape JSON.");
        return;
      }
      if (!entries.length) {
        alert("No valid models found on the clipboard.");
        return;
      }
      let firstIndex = null;
      const addedIndices = [];
      ...
      console.log(`[Blockscape] toolbar pasted ${entries.length} model(s)`);
      if (firstIndex != null) setActive(firstIndex);
      if (await isLocalBackendReady()) {
        await autoSaveNewLocalFilesForModels(addedIndices, { origin: "clipboard" });
      }
    });
  }
